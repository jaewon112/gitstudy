...
public class LambdaInterface {
    public static void main(String[] args) {
        Runnable runnable = () -> {
            System.out.println("Runnable 은 메인문처럼 쓸 수 있음");
        };
        runnable.run();
        ...
        // apply 는 추상메서드로 정의
        String nameText = fx.apply("1");
        System.out.println(nameText);

        UnaryOperator<String> fx2 = name -> "이름: " + name;
        nameText = fx2.apply("1");
        System.out.println(nameText);

        String searchName = "1";
        List<String> 일학년이름 = List.of("1", "2", "3");
        List<String> 이학년이름 = List.of("4", "5", "6");
        ...
        System.out.println(isContains2.test(searchName));
...
//        Stream<Integer> numStream = List.of(1, 2, 3, 4).stream();
        ...
        numStream.filter(num -> num % 2 == 0).forEach(n -> System.out.print(n+ ","));
